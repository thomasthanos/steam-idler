...
// src/main/steam/worker.ts
var worker_exports = {};
__export(worker_exports, {
  STATS_NOT_RECEIVED_SENTINEL: () => STATS_NOT_RECEIVED_SENTINEL
});
module.exports = __toCommonJS(worker_exports);
var sw = __toESM(require("steamworks.js"));
...
async function fetchSchema(appId, apiKey) {
  const url2 = `https://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v2/?key=${apiKey}&appid=${appId}&l=english`;
  const res = await axios_default.get(url2, { timeout: 1e4 });
...
function handleInit(id, appId, apiKey) {
  try {
    ...
    send(id, false, void 0, e.message);
  }
}
async function handleGetAchievements(id) {
  ...
  if (!storedApiKey) {
    send(id, false, void 0, "Steam API key required to load achievements. Add one in Settings.");
    return;
  }
  try {
    ...
    let globalPct = {};
    try {
      const url2 = `https://api.steampowered.com/ISteamUserStats/GetGlobalAchievementPercentagesForApp/v2/?gameid=${currentAppId}`;
      const res = await axios_default.get(url2, { timeout: 6e3 });
    ...
    try {
      const steamId = client.localplayer.getSteamId().steamId64.toString();
      const url2 = `https://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v1/?key=${storedApiKey}&steamid=${steamId}&appid=${currentAppId}&l=english`;
      const res = await axios_default.get(url2, { timeout: 8e3 });
    ...
    const achievements = schema.achievements.map((s) => {
      ...
      const unlocked = web !== void 0 ? web.achieved : client.achievement.isActivated(s.name);
      const unlockedAt = web?.unlocktime && web.unlocktime > 0 ? web.unlocktime : void 0;
      return {
        apiName: s.name,
...
var STATS_NOT_RECEIVED_SENTINEL = "STATS_NOT_RECEIVED";
async function handleSetAchievement(id, apiName, unlocked) {
  ...
  try {
    ...
    while (attempt < MAX_ATTEMPTS) {
      ok = unlocked ? client.achievement.activate(apiName) : client.achievement.clear(apiName);
      ...
      if (attempt === FAST_FAIL_ATTEMPTS) {
        ...
        send(id, false, void 0, STATS_NOT_RECEIVED_SENTINEL);
        return;
      }
      await new Promise((r) => setTimeout(r, RETRY_DELAY));
    }
    if (!ok) {
      send(id, false, void 0, `Steam rejected the achievement update after ${MAX_ATTEMPTS} attempts. Make sure Steam is running and the game supports achievements.`);
      return;
    }
...
async function handleSetAllAchievements(id, unlocked) {
  ...
  if (!storedApiKey) {
    send(id, false, void 0, "Steam API key required. Add one in Settings.");
    return;
  }
  try {
    const schema = await fetchSchema(currentAppId, storedApiKey);
    ...
    for (let i = 0; i < MAX_PUMP; i++) {
      ...
      if (schema.achievements.length > 0) {
        const test2 = unlocked ? client.achievement.activate(schema.achievements[0].name) : client.achievement.clear(schema.achievements[0].name);
        if (test2) {
          pumped = true;
          break;
        }
    ...
    if (!pumped && schema.achievements.length > 0) {
      send(id, false, void 0, "Steam stats not ready. Make sure Steam is running.");
      return;
    }
